...
namespace NeeqDMIs.Keyboard
{
    public sealed class KeyboardModule
    {
        ...
        private Thread behaviorsThread = new Thread(new ParameterizedThreadStart(SendEvent));
        /// <summary>
        ...
        private static List<AKeyboardBehavior> keyboardBehaviors = new List<AKeyboardBehavior>();
        /// <summary>
        /// Keeps track of the last input event, for debug purposes.
        /// </summary>
        private RawInputEventArg lastInputEventArg;
        #endregion
        ...
        /// </summary>
        public static List<AKeyboardBehavior> KeyboardBehaviors { get => keyboardBehaviors; set => keyboardBehaviors = value; }

        private void OnKeyPressed(object sender, RawInputEventArg e)
        {
            lastInputEventArg = e; // Keeps track of the last event, for debug purposes.
            SendEvent(e);
        }

        private static void SendEvent(object obj)
        {
            RawInputEventArg ev = (RawInputEventArg)obj;
            foreach(AKeyboardBehavior behavior in KeyboardBehaviors)
            {
                behavior.ReceiveEvent(ev);
            }
        }
